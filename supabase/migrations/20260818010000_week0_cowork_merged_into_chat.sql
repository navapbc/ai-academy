-- week0_cowork_merged_into_chat — corrected Cowork description after Anthropic merged Chat and Cowork into one surface.
--
...
begin
  ...
  if missing is not null then
    raise notice 'week0_cowork_merged_into_chat: % target cell(s) absent, their updates will affect 0 rows: %',
      array_length(missing, 1), array_to_string(missing, ', ');
  end if;
end $$;

-- c1-w0-claude-setup — Claude Set-up
-- Cowork is no longer a separate sidebar space — it is a mode toggle inside Chat, on web, desktop, and mobile
...
begin
  ...
  raise notice 'week0_cowork_merged_into_chat: reconciled % of 1 reviewed cell(s).', updated;
end $$;
