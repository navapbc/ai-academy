-- week0_login_sso — corrected Week 0 sign-in instructions.
--
...
begin
  ...
  if missing is not null then
    raise notice 'week0_login_sso: % target cell(s) absent, their updates will affect 0 rows: %',
      array_length(missing, 1), array_to_string(missing, ', ');
  end if;
end $$;

-- c1-w0-claude-setup — Claude Set-up
-- Nava Claude accounts are reached via email → Continue with SSO, not Continue with Google
...
begin
  ...
  raise notice 'week0_login_sso: reconciled % of 1 reviewed cell(s).', updated;
end $$;
