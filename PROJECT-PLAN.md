...
| W2 | Bug fixes | **W2-1 DONE (#59)** ✓ user-keyed cache+outbox, legacy keys dropped, sign-out hygiene, 2-user regression test (D-01 closed); **W2-2** role-escalation guard migration + RLS test (D-06) — *needs human: access-control*; **W2-3** 2.1 single-gate (D-02) — *needs human: which gate wins*; **W2-4** error-path cluster (D-03/04/05/13/15) one PR ← **next**; **W2-5** DATA-01 test reconciled + `RUN_DB_TESTS` CI job (D-07/24); **W2-6** PairedCalibration cluster (D-09/14/18); **W2-7** config shape-guards (D-16); **W2-8** retry-grading (D-17); **W2-9** a11y parity (D-10/19/20) | P1–P3 | S–M each | W2-1 done; W2-4 next |
