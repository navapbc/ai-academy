...
Nava AI Academy — an internal AI-literacy training platform that teaches the Nava AI
Literacy Skills Matrix (Stages 1–2). A React 19 + Vite SPA backed by Claude (via API)
and a Supabase data layer. Runs fully locally in development; deploys last (Phase 7).
...
Run a single test: `npx vitest run src/lib/gating.test.ts` or filter by name with `-t "substring"`.
...
matrix cell, and is fetched at runtime (`src/lib/useCurriculum.ts` → `src/lib/modules.ts`, which
maps rows to the `Module`/`Phase` types in `src/types.ts`). Quizzes come from each row's
`quiz_json`, lab config from `lab_config_json`, sorter config from `sorter_config_json`. Editing a
row changes a lesson with no rebuild. The former static seed files (`src/content/*.md`,
`data/phases.ts`, `data/quiz.ts`) were removed. New/changed curriculum is seeded via
`supabase/migrations/*seed*.sql` and `supabase/seed-data/curriculum-content.json`; see
`docs/content-guide.md` for the authoring workflow.

### App flow
`App.tsx`: `useAuth` → (Login | curriculum-gated `Academy`). `Academy` flattens phases, computes
gating, and owns `useProgress`. An empty `modules` table is treated as an error state (not a crash —
guard added for audit FE-02).
...
### Gating (`src/lib/gating.ts`)
Stage 2 is locked until **all** of Stage 1a is complete; Stage 1b is never gated. Pure functions
over `(phases, completedModuleIds)` — no schema/auth involvement. `completeModule` skips locked
modules when advancing the cursor.

### Exercises & grading
`ModuleRenderer.tsx` dispatches on the module's `labConfig.kind` (a discriminated union in
`types.ts`) to one of the `src/components/exercises/*` components. **Add new exercise types
additively** as new union members + a new switch case, so parallel PRs merge cleanly. A module's
completion gate is usually its quiz; cell 2.1 is the deliberate exception (the lab gates, decision
D8). Two graders produce the same `GradeResult` shape: the LLM judge and a pure auto-key grader
(`src/lib/grading/autoGrade.ts`). `useLabGrading.ts` is the shared judge-grading hook (state +
in-place retry of an already-saved submission) used by all judge-graded labs.
