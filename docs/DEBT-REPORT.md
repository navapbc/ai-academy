...
- **LLM-10** — Models are unpinned aliases, not dated snapshots — `chat/index.ts:16`, `src/lib/models.ts:13-16`. The ids (`claude-haiku-4-5`, `claude-sonnet-4-6`) are plausibly current; the issue is reproducibility/stability of floating aliases (graded lab transcripts become non-reproducible). *Direction:* pin dated snapshots with a deliberate upgrade step.
  - **☑ Accepted** (`fix/p3-final`): accepted — unpinned aliases are a deliberate auto-upgrade choice for an internal tool; pinning to dated snapshots is deferred as an ops decision (noted for reproducibility-sensitive use). Re-confirmed deferred in P6.1 (pinning excluded by decision; unpinned auto-upgrade aliases retained).
...
- **A11Y-02 — Modals have no focus trap, no Escape, no focus move-in/restore, no dialog role** — `SupportModal.tsx:19-108`, `LocalTutorFAB.tsx:16-55`. SC 2.4.3, 4.1.2, 2.1.1. *Direction:* `role=dialog`/`aria-modal`, focus management, trap, Escape, restore (consider a headless dialog primitive).
  - **✅ Resolved** (`fix/a11y-modals-contrast`): added a reusable `useDialogA11y` hook (focus move-in, Tab trap, Escape, focus restore); SupportModal is `role=dialog aria-modal aria-labelledby` and the FAB popover is `role=dialog aria-label`; icon-only close/toggle buttons got `aria-label`. Test: `SupportModal.test.tsx`.
